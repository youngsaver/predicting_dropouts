...
install.packages("doFuture") # parallel processing
install.packages("foreach") # parallel looping structure
...
power to our final model. The reason we can compare the size of the predictors
is that we've already standardized our predictors, so they operate on the same
scale (standard deviation of 1). The larger the predictor coefficient, the more 
the variation in the response variable can be explained by a 1 standard 
deviation change in that predictor (or, in the case of indicator predictors, 
by a change in the indicator class).
...
variable, X, and there is one outcome variable, Y. Here is the data:
...
lasso_results <- foreach(d = 1:length(lambdas), .combine = rbind) %dopar% {
 ...
 for(i in 1:K){
   ...
   predicted <- ifelse(probabilities > 0.50, 1, 0)
...
We can see here that relatively low parameter values perform better, in terms of
overall accuracy. However, our cross-validation shows that setting the parameter
value all the way to zero, which would be equivalent to logistic regression, is
not optimal. By having a small but present penalty factor, our model corrects
...
We see a similar shape; however, the y-axis scale has changed. The accuracy 
among dropouts is incredibly low. Let's compare the best overall accuracy and
best dropout dropout accuracy during cross-validation, as well as their
associated $\lambda$ values.

```{r}
#Stores best overall accuracy and lambda value
...
#Stores best dropout accuracy and lambda value
...
cross.results <- data.frame(accuracy = c(best.overall.accuracy,
...
produced was `r best.overall.accuracy`. By contrast, our best performance among 
the dropouts was an accuracy of only `r best.dropout.accuracy`. With the
particularly dismal performance among the dropouts, we need to rethink how we
do our cross-validation, to take into account the fact that one class (dropouts)
has far fewer data points than the other class (non-dropouts). How can we fit our
model so that it's more sensitive to the minority class (dropouts) while also
making sure it doesn't provide too many false alarms (i.e., predicting a 
dropout when the student wouldn't have otherwise dropped out)? The next section
...
<img src="../img/roc_example.png" style="display: block; margin: 0 auto; height: 320px;">
...
The numbers printed above are the coefficient values for our predictors. Note 
that the Lasso tuned some of these coefficient values to zero, meaning the 
predictors were effectively dropped from our model. Thus, we can see how the 
Lasso's penalty factor not only diminishes coefficient values, but also drops 
some variables from the model completely, in order to simplify and prevent 
overfitting.
